...
function(x, keyVars=1:3 , w=4){
  ...
  z <- aggregate(xx$w, list(xx$indexG), sum)
  ...
  n1 <- dim(z[z[,P]==1,])[1]
  n2 <- dim(z[z[,P]==2,])[1]
  z <- list(freqCalc=z[, -c(1,which(colnames(z)=="Freq"),ncol(z)-1,ncol(z))][,c(cn2)], keyVars=keyVars, w=w, indexG=z$indexG, fk=z$fk, Fk=z$Fk, n1=n1, n2=n2)
  class(z) <- "freqCalc"
  invisible(z)
}
